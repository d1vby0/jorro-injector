            ...
            foreach ($parameters ?? $function->getParameters() as $index => $parameter) {
                ...
                if ($attribute = $attributes[$name] ?? null) {
                    ...
                    if ($attribute->hasPlugins()) {
                        if (!$attribute->applyPlugins($injectFrom, $parameter, $injectArgs, $args)) {
                    ...
                    try {
                        if ($attribute->id) {
                            $args[$name] = $this->container->getInstance($attribute->id, $attribute->lazy, $parameter, ... $injectArgs);
                            continue;
                        }
                        $types = $parameter->getType();
                        if ($types instanceof \ReflectionNamedType) {
                            if (!$types->isBuiltin()) {
                                $args[$name] = $this->container->getInstance($types->getName(), $attribute->lazy, $parameter, ... $injectArgs);
                                continue;
                            }
                        } else {
                            if ($types) {
                                foreach ($types->getTypes() as $type) {
                                    if (!$type->isBuiltin()) {
                                        try {
                                            $args[$name] = $this->container->getInstance($type->getName(), $attribute->lazy, $parameter, ... $injectArgs);
                                            continue 2;
                                        } catch (NotFoundExceptionInterface $e) {
                                        }
                                    }
